...
export class WatchService {
  ...
  async getWatches(params: {
    ...
    if (diameterRange) {
      const [min, max] = diameterRange.split("-").map(Number);
      where.diameter = {
        ...(min && { gte: min }),
        ...(max && { lte: max }),
      };
    ...
    if (waterResistanceRange) {
      const [min, max] = waterResistanceRange.split("-").map(Number);
      where.waterResistance = {
        ...(min && { gte: min }),
        ...(max && { lte: max }),
      };
    }
